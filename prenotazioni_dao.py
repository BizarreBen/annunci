...
class Prenotazione():
    ...
    def __init__(self, id_prenotazione, id_annuncio, email_utente, fascia_oraria, data, virtuale, status, annuncio: annunci_dao.Annuncio = None):
...
def get_prenotazioni(email, limit = 0):
    ...
    with sqlite3.connect(db_location) as connection:
        ...
        try:
            ...
            return [Prenotazione(prenotazione['id'], prenotazione['id_annuncio'], prenotazione['email_utente'], prenotazione['fascia_oraria'], prenotazione['data'], prenotazione['virtuale'], prenotazione['status'],
                                 annunci_dao.Annuncio(prenotazione['id_annuncio'], prenotazione['Titolo'], prenotazione['Indirizzo'], prenotazione['Tipo'], prenotazione['Locali'],
                                                      prenotazione['Descrizione'], prenotazione['Prezzo'], prenotazione['Arredata'], prenotazione['Id_Locatore'], prenotazione['Visibile'], prenotazione['Immagini'])
...
def update_status_prenotazione(id, email, status):
    query = '''
            UPDATE prenotazioni SET status = ? WHERE id = ? AND EXISTS (SELECT Id_locatore FROM annunci WHERE Id == id_annuncio AND Id_locatore == ?)
            '''
    
    with sqlite3.connect(db_location) as connection:
        cursor = connection.cursor()
        try:
            cursor.execute(query, (status, id, email))
            if cursor.rowcount == 0:
