    ...
    with sqlite3.connect(db_location) as connection:
        ...
        try:
            ...
            for immagine in immagini:
                with Image.open(immagine) as image:
                    background = Image.new(
                        "RGBA",
                        size=(
                            (image.width, image.width * 9 // 16)
                            if image.width / image.height > 16 // 9
                            else (image.height * 16 // 9, image.height)
                        ),
                        color=(0, 0, 0, 0),
                    )
                    img_w, img_h = image.size
                    bg_w, bg_h = background.size
                    background.paste(image, ((bg_w - img_w) // 2, (bg_h - img_h) // 2))
                    background.save(
                        os.path.join(
                            filepath,
            ...
            if len(immagini) > 0 and not immagini[0] == "":
                for immagine in immagini:
                    with Image.open(immagine) as image:
                        ...
                        bg_w, bg_h = background.size
                        background.paste(
                            image, ((bg_w - img_w) // 2, (bg_h - img_h) // 2)
                        )
                        background.save(
                            os.path.join(
                                filepath,
